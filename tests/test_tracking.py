...
class TrackingTests(unittest.TestCase):
    ...
    def test_continue_after_target_holds_position(self) -> None:
        tracker = PortfolioTracker(initial_capital=1000, target_profit_pct=0.1, max_loss_pct=0.05)
        tracker.record_trade("buy", 100, 5)
        tracker.record_trade("sell", 120, 5)
        # With continue_after_target defaulting to True, hitting the target should NOT stop the bot
        self.assertIsNone(tracker.stop_reason(120))
